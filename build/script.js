...
(function(window, document, undefined) {
    ...
    function minErr(module, ErrorConstructor) {
        ...
        return function() {
            ...
            message = message + "\nhttp://errors.angularjs.org/1.3.4/" + (module ? module + "/" : "") + code;
...
(function(window, angular, undefined) {
    ...
    angular.module("ct.ui.router.extras").service("$deepStateRedirect", [ "$rootScope", "$state", "$injector", function($rootScope, $state, $injector) {
        ...
        function getConfig(state) {
            ...
            if (angular.isFunction(declaration)) dsrCfg.fn = declaration; else if (angular.isObject(declaration)) dsrCfg = angular.extend(dsrCfg, declaration);
            if (!dsrCfg.fn) {
        ...
        function getParamsString(params, dsrParams) {
            ...
            angular.forEach(dsrParams.sort(), function(name) {
                paramsToString[name] = safeString(params[name]);
            });
        ...
        return {
            reset: function(stateOrName) {
                if (!stateOrName) {
                    ...
                    if (lastSubstate[state.name]) lastSubstate[state.name] = {};
                }
            }
        };
    } ]);
        ...
        this.$get = [ "$rootScope", "$state", "$stateParams", "$injector", "$log", function($rootScope, $state, $stateParams, $injector, $log) {
            ...
            var stickySupport = {
                ...
                processTransition: function(transition) {
                    ...
                    var fromPath = transition.fromState.path, fromParams = transition.fromParams, toPath = transition.toState.path, toParams = transition.toParams, options = transition.options;
                    ...
                    for (idx = keep; idx < toPath.length; idx++) {
                        ...
                        if (enterTrans == "reactivate") deepestReactivate = reactivatedStatesByName[toPath[idx].name] = toPath[idx];
                        if (enterTrans == "updateStateParams") deepestUpdatedParams = toPath[idx];
                    }
                    ...
                    angular.forEach(keptStateNames, function(name) {
                        ...
                        for (var i = 0; inactiveChildren && i < inactiveChildren.length; i++) {
                            ...
                            if (!reactivatedStatesByName[child.name] && (!deepestReactivate || child.self.name.indexOf(deepestReactivate) !== 0) && (!deepestUpdatedParams || child.self.name.indexOf(deepestUpdatedParams) !== 0)) result.inactives.push(child);
                        }
                    });
    ...
    angular.module("ct.ui.router.extras").config([ "$provide", "$stateProvider", "$stickyStateProvider", "$urlMatcherFactoryProvider", function($provide, $stateProvider, $stickyStateProvider, $urlMatcherFactoryProvider) {
    ...
    angular.module("ct.ui.router.extras").provider("$futureState", [ "$stateProvider", "$urlRouterProvider", "$urlMatcherFactoryProvider", function _futureStateProvider($stateProvider, $urlRouterProvider, $urlMatcherFactory) {
    ...
    angular.module("ct.ui.router.extras").run([ "$futureState", function($futureState) {} ]);
    angular.module("ct.ui.router.extras").service("$previousState", [ "$rootScope", "$state", function($rootScope, $state) {
        var previous = null;
        var memos = {};
...
angular.module("states", []).run(function() {}).config(function($stateProvider, $urlRouterProvider, $locationProvider, $httpProvider) {
...
angular.module("grid", []).controller("gridController", function($scope, $timeout) {
    var dir = {
        N: {
            x: 0,
            y: 1
    ...
    function createPatches(grid, patches) {
        for (var i = 0; i < patches.length; i++) {
            grid[patches[i].y].value[patches[i].x] = 2;
        }
        return grid;
    }
    ...
    function process(step) {
        if (step != $scope.instructions.length) {
            var nextStep = dir[$scope.instructions[step]];
            var newPos = {
            ...
            if (newPos.x < $scope.grid[0].value.length && newPos.y < $scope.grid.length) {
                ...
                if ($scope.grid[$scope.currentPos.y].value[$scope.currentPos.x]) {
                    $scope.totalCleaned++;
                }
    ...
    var emptyGrid = createGrid(5, 5), patches = [ {
        x: 1,
        y: 3
    }, {
        x: 3,
...
angular.module("home", []).controller("homeController", function($scope, $timeout) {
    $scope.input = "";
    $scope.error = "";
    var letters = [ "N", "S", "E", "W" ];
    $scope.process = function() {
        try {
            ...
            if (lines.length > 3) {
                ...
                if (instructions.length < 1) throw "invalid instructions";
                for (var i = 0; i < instructions.length; i++) {
                    if (letters.indexOf(instructions[i]) < 0) throw "invalid instructions";
                }
                var size = lines.splice(0, 1)[0].split(" ");
                if (size.length != 2 || isNaN(size[0]) || isNaN(size[1])) throw "invalid grid size";
                size[0] = parseInt(size[0]);
                size[1] = parseInt(size[1]);
                var pos = lines.splice(0, 1)[0].split(" ");
                if (pos.length != 2 || isNaN(pos[0]) || isNaN(pos[1]) || pos[0] >= size[0] || pos[1] >= size[1]) throw "invalid initial position";
                pos = {
                    x: parseInt(pos[0]),
                    y: parseInt(pos[1])
                };
...
angular.module("app", [ "ui.router", "templates", "breakpointApp", "ngAnimate", "states", "services", "home" ]).config(function() {}).controller("appController", function($scope) {
    var $this = this;
